...
describe('react-inject-env', () => {
  describe('Build', () => {
    ...
    it('should not overwrite variables that are not passed in', () => {
      const file = buildFile()
      expect(file).toContain('VITE_NO_INPUT: \n')
    })
    ...
    //   const file = readFile('tests/output/test.txt')
    //   expect(file).toContain('VITE_DOT_ENV: \n')
    // })

    it('should build with actual variables when using --bypass', () => {
      const time = new Date().toISOString()
      process.env['VITE_BYPASS_ENV'] = time
      const commandLine = new InjectEnvCommandLine()
      commandLine.execute(['build', '--bypass', 'VITE_BYPASS_ENV', 'sh', 'tests/scripts/buildFile.sh'])
      const file = readFile('tests/output/test.txt')
      expect(file).toContain(`VITE_BYPASS_ENV: ${time}`)
    })
  })
  ...
  describe('Inject', () => {
    ...
    it('should replace file with injected env variables from dotenv', () => {
      shell.exec('echo VITE_INJECT_ENV2 = B > .env')
      buildFile()
      const commandLine = new InjectEnvCommandLine()
      commandLine.execute(['inject', '-d', 'tests/output'])
      const file = readFile('tests/output/test2.txt')
      expect(file).toContain('VITE_INJECT_ENV2: B')
    })
    ...
    it('should replace all files in directory', () => {
      process.env['VITE_INJECT_ENV3'] = 'C'
      buildFile()
      const commandLine = new InjectEnvCommandLine()
      commandLine.execute(['inject', '-d', 'tests/output'])
      expect(readFile('tests/output/test3/test3.txt')).toContain('VITE_INJECT_ENV3: C')
      expect(readFile('tests/output/test3/test3b.txt')).toContain('VITE_INJECT_ENV3: C')
    })

    it('should output to new folder without modifying the origin when using --o', () => {
      process.env['VITE_INJECT_ENV4'] = 'D'
      buildFile()
      const commandLine = new InjectEnvCommandLine()
  ...
  describe('Set', () => {
    ...
    it('should generate env file based on process.env variables', () => {
      process.env['VITE_SET_ENV1'] = 'ENV1'
      const commandLine = new InjectEnvCommandLine()
      commandLine.execute(['set', '-d', './tests/output3', '-n', 'env3.js'])
      expect(readFile('tests/output3/env3.js')).toContain('"VITE_SET_ENV1": "ENV1"')
    })
    it('should generate env file based on dotenv variables', () => {
      const commandLine = new InjectEnvCommandLine()
      commandLine.execute(['set', '-d', './tests/output3', '-n', 'env4.js'])
      expect(readFile('tests/output3/env4.js')).toContain('"VITE_SET_ENV2": "C"')
    })
